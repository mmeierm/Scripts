###### Prepare main UI########

write-host "Preparing UI"

[void] [System.Reflection.Assembly]::LoadWithPartialName("System.Drawing")
[void] [System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms")
[System.Windows.Forms.Application]::EnableVisualStyles();
$objForm = New-Object System.Windows.Forms.Form
$objForm.Backcolor="white"
$objForm.Size = New-Object System.Drawing.Size(1000,550)
$objForm.Text = "User driven GroupMembership"
#$objForm.Icon="$PSScriptRoot\Icon.ico"
$Workbook="<INSERT Webhook URI here>"
$objForm.MaximizeBox = $false

###### Getting computer information########
$name=$env:COMPUTERNAME
$Manufacrurer=(Get-WmiObject -Class win32_computersystem).Manufacturer
If ($Manufacrurer -notlike "*Lenovo*")
{
$Model=(Get-WmiObject -Class win32_computersystem).Model
}
else
{
$Model=(Get-WmiObject -Class win32_computersystemproduct).Version
}





###### Getting DSREGCMD Status########

$DsregCmdStatus = dsregcmd /status
if($DsregCmdStatus -match "DeviceId")
{
$DeviceId = $DsregCmdStatus -match "DeviceID"
$DeviceId = ($DeviceId.Split(":").trim())
$DeviceId = $DeviceId[1]
}



###### OptIn########
$EnablementBTN = New-Object System.Windows.Forms.Button
# Die nächsten beiden Zeilen legen die Position und die Größe des Buttons fest
$EnablementBTN.Location = New-Object System.Drawing.Size(50,300)
$EnablementBTN.Size = New-Object System.Drawing.Size(400,55)
$EnablementBTN.Text = "Opt-In"
$EnablementBTN.Name = "EnablementBTN"
$EnablementBTN.DialogResult = "Cancel"
$EnablementBTN.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 18, [System.Drawing.FontStyle]::Bold)
#Die folgende Zeile ordnet dem Click-Event die Schließen-Funktion für das Formular zu
$EnablementBTN.Add_Click({



$body = @{
    ID="$DeviceId"
    Name="$env:computername"
    Action="OPTIN"
}
$json = $body | ConvertTo-Json

try {
    Invoke-RestMethod -Method Post -Uri $Workbook -Body $json -ContentType 'application/json'
    [System.Windows.Forms.MessageBox]::Show(„Enrollment for this computer has been successfully initiated“,“Enablement successful“,0)
} catch {
   [System.Windows.Forms.MessageBox]::Show(„Enrollment for this computer could not be initiated successfully. Please ensure your computer is connected to internet and retry later.“,“Enrollment could not be completed“,0)
}

})


########### OPT OUT###########
$DisablementBTN = New-Object System.Windows.Forms.Button
# Die nächsten beiden Zeilen legen die Position und die Größe des Buttons fest
$DisablementBTN.Location = New-Object System.Drawing.Size(550,300)
$DisablementBTN.Size = New-Object System.Drawing.Size(400,55)
$DisablementBTN.Text = "Opt-Out"
$DisablementBTN.Name = "DisablementBTN"
$DisablementBTN.DialogResult = "Cancel"
$DisablementBTN.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 18, [System.Drawing.FontStyle]::Bold)
#Die folgende Zeile ordnet dem Click-Event die Schließen-Funktion für das Formular zu
$DisablementBTN.Add_Click({



$body = @{
    ID="$DeviceId"
    Name="$env:computername"
    Action="OPTOUT"
}
$json = $body | ConvertTo-Json


try {
    Invoke-RestMethod -Method Post -Uri $Workbook -Body $json -ContentType 'application/json'
    [System.Windows.Forms.MessageBox]::Show(„Unenrollment for this computer has been successfully initiated“,“Unenrollment successful“,0)
} catch {
   [System.Windows.Forms.MessageBox]::Show(„Unenrollment for this computer could not be initiated successfully. Please ensure your computer is connected to internet and retry later.“,“Unenrollment could not be completed“,0)
}





})



################ Finish UI######


$base64ImageString = ""
$imageBytes = [Convert]::FromBase64String($base64ImageString)
$ms = New-Object IO.MemoryStream($imageBytes, 0, $imageBytes.Length)
$ms.Write($imageBytes, 0, $imageBytes.Length);
$img = [System.Drawing.Image]::FromStream($ms, $true)
$pictureBox = new-object Windows.Forms.PictureBox
$pictureBox.Location = New-Object System.Drawing.Size(0,1)
$pictureBox.Size = New-Object System.Drawing.Size(437,135)
$pictureBox.Dock = [System.Windows.Forms.DockStyle]::Top
$pictureBox.SizeMode = [System.Windows.Forms.PictureBoxSizeMode]::CenterImage
$pictureBox.Image = $img

$Label0 = New-Object System.Windows.Forms.Label
$Label0.Location = New-Object System.Drawing.Size(0,170)
$Label0.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 18, [System.Drawing.FontStyle]::Bold)
$Label0.ForeColor = "Black"
$Label0.Text = "User driven GroupMembership"
$Label0.Width = 1000
$label0.Height = 50
$label0.TextAlign="TopCenter"


$Label1 = New-Object System.Windows.Forms.Label
$Label1.Location = New-Object System.Drawing.Size(20,250)
$Label1.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 11, [System.Drawing.FontStyle]::Bold)
$Label1.ForeColor = "Black"
$Label1.Text = "Your computer information: $Manufacrurer $Model"
$Label1.Width = 1000
$label1.Height = 50
$Label1.AutoSize = $False
$label1.TextAlign="TopCenter"

<# 
#To be done in a future release
$Label2 = New-Object System.Windows.Forms.Label
$Label2.Location = New-Object System.Drawing.Size(25,400)
$Label2.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 11)
$Label2.ForeColor = "Black"
$Label2.Text = "Current Enrollment Status: $Status "
$Label2.AutoSize = $False
$Label2.Width = 950
$label2.Height = 50
$label2.TextAlign="TopCenter"
#>




$Label3 = New-Object System.Windows.Forms.Label
$Label3.Location = New-Object System.Drawing.Size(25,450)
$Label3.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 11)
$Label3.ForeColor = "Black"
$Label3.Text = "Click the opt-in button, to enroll this computer. `n Click the opt-out button for unenrollment of this computer. "
$Label3.AutoSize = $False
$Label3.Width = 950
$label3.Height = 200
$label3.TextAlign="TopCenter"


$objForm.Controls.Add($Label0)
$objForm.Controls.Add($Label1)
#$objForm.Controls.Add($Label2)
$objForm.Controls.Add($Label3)
$objForm.controls.add($pictureBox)
$objForm.Controls.Add($EnablementBTN)
$objForm.Controls.Add($DisablementBTN)
$objForm.controls.add($pictureBoxResult)


######## Hide PowerShell Console#######
Add-Type -Name Window -Namespace Console -MemberDefinition '
[DllImport("Kernel32.dll")]
public static extern IntPtr GetConsoleWindow();
[DllImport("user32.dll")]
public static extern bool ShowWindow(IntPtr hWnd, Int32 nCmdShow);
'
$consolePtr = [Console.Window]::GetConsoleWindow()
[Console.Window]::ShowWindow($consolePtr, 0)

[void] $objForm.ShowDialog()


