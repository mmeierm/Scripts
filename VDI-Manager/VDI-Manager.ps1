...
#region Get VM Inventory


###############################02_VMs Folder Inventory###############################
$VMPath = "C:\02_VMs"
If (Test-Path $VMPath)
    {
...
try
    {
    #try to get the vmdk of VMs outside 02_VMs
...
[System.Collections.ArrayList]$offlineVMs = $VMiniArray

#endregion Get VM Inventory

##############################################################
If(! (Test-Path -Path "C:\ProgramData\VDI-Manager\VDI-Manager-VMs.json"))
{
    New-Item -Path "C:\ProgramData\VDI-Manager\VDI-Manager-VMs.json" -ItemType File -Force
}

Set-Content -Path "C:\ProgramData\VDI-Manager\VDI-Manager-VMs.json" -Value ($VMiniArray | ConvertTo-Json -Depth 10)

Invoke-WebRequest -Uri "<INSERT URL HERE>" -OutFile "C:\ProgramData\VDI-Manager\VDI-Manager-Versions.json" -UseBasicParsing
...
{  
###############################GUI Table View###############################
&$PSScriptRoot\VDI-Manager-GUI.ps1
}
else
{
&$PSScriptRoot\VDI-Manager-Notifications.ps1
}
