$Workbook="<INSERT Webhook URI here>"
$GroupID="<INSERT Opt-In Group ID here>"
$base64ImageString = ""


###### Prepare main UI########

write-host "Preparing UI"

[void] [System.Reflection.Assembly]::LoadWithPartialName("System.Drawing")
[void] [System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms")
[System.Windows.Forms.Application]::EnableVisualStyles();
$objForm = New-Object System.Windows.Forms.Form
$objForm.Backcolor="white"
$objForm.Size = New-Object System.Drawing.Size(500,450)
$objForm.Text = "Temporary Group Membership"
#$objForm.Icon="$PSScriptRoot\Icon.ico"


###### Getting computer information########
###########################################################################
$name=$env:COMPUTERNAME
$SN=(Get-WmiObject -Class win32_bios).SerialNumber
$Manufacrurer=(Get-WmiObject -Class win32_computersystem).Manufacturer
If ($Manufacrurer -notlike "*Lenovo*")
{
$Model=(Get-WmiObject -Class win32_computersystem).Model
}
else
{
$Model=(Get-WmiObject -Class win32_computersystemproduct).Version
}
#############################################################################
$EnablementBTN = New-Object System.Windows.Forms.Button
$EnablementBTN.Location = New-Object System.Drawing.Size(20,350)
$EnablementBTN.Size = New-Object System.Drawing.Size(450,55)
$EnablementBTN.Text = "Add to Group"
$EnablementBTN.Name = "EnablementBTN"
$EnablementBTN.DialogResult = "Cancel"
$EnablementBTN.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 18, [System.Drawing.FontStyle]::Bold)
$EnablementBTN.Add_Click({

$DsregCmdStatus = dsregcmd /status
if($DsregCmdStatus -match "DeviceId")
{
$DeviceId = $DsregCmdStatus -match "DeviceID"
$DeviceId = ($DeviceId.Split(":").trim())
$DeviceId = $DeviceId[1]
}


$body = @{
    ID="$DeviceId"
    Name="$env:computername"
	Duration=$($TimeDropdown.SelectedItem)
	GroupID="$GroupID"
}
$json = $body | ConvertTo-Json

try {
	Invoke-RestMethod -Method Post -Uri $Workbook -Body $json -ContentType 'application/json'
	[System.Windows.Forms.MessageBox]::Show("Opt-In for this computer has been successfully initiated","Opt-In successful",0)
	}
	catch 
	{
   [System.Windows.Forms.MessageBox]::Show("Opt-in for this computer could not be initiated successfully. Please ensure your computer is connected to internet and retry later.","Opt-In could not be completed",0)
	}


})

$objForm.Controls.Add($EnablementBTN)



###########################################################################


$imageBytes = [Convert]::FromBase64String($base64ImageString)
$ms = New-Object IO.MemoryStream($imageBytes, 0, $imageBytes.Length)
$ms.Write($imageBytes, 0, $imageBytes.Length);
$img = [System.Drawing.Image]::FromStream($ms, $true)
$pictureBox = new-object Windows.Forms.PictureBox
$pictureBox.Location = New-Object System.Drawing.Size(0,1)
$pictureBox.Size = New-Object System.Drawing.Size(515,150)
$picturebox.sizemode=1
$pictureBox.Image = $img

$Label0 = New-Object System.Windows.Forms.Label
$Label0.Location = New-Object System.Drawing.Size(20,170)
$Label0.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 18, [System.Drawing.FontStyle]::Bold)
$Label0.ForeColor = "Black"
$Label0.Text = "Temporary Group Membership"
$Label0.AutoSize = $True

$Label1 = New-Object System.Windows.Forms.Label
$Label1.Location = New-Object System.Drawing.Size(20,210)
$Label1.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 11, [System.Drawing.FontStyle]::Bold)
$Label1.ForeColor = "Black"
$Label1.Text = "$Manufacrurer $Model"
$Label1.AutoSize = $True

$Label2 = New-Object System.Windows.Forms.Label
$Label2.Location = New-Object System.Drawing.Size(20,240)
$Label2.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 11)
$Label2.ForeColor = "Black"
$Label2.Text = "Select the duration, where the device with Name: $name"
$Label2.AutoSize = $True

$Label3 = New-Object System.Windows.Forms.Label
$Label3.Location = New-Object System.Drawing.Size(20,260)
$Label3.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 11)
$Label3.ForeColor = "Black"
$Label3.Text = "and SN: $SN should be exluded."
$Label3.AutoSize = $True


$TimeDropdown=New-object system.windows.forms.combobox
$TimeDropdown.Location = New-Object System.Drawing.Size(20,300)
$TimeDropdown.Size = New-Object System.Drawing.Size(450,45)
$TimeDropdown.Items.Add("1 Day") | Out-Null
$TimeDropdown.Items.Add("3 Days") | Out-Null
$TimeDropdown.Items.Add("7 Days") | Out-Null
$TimeDropdown.Selectedindex =$TimeDropdown.FindString('1 Day')
$TimeDropdown.Font = [System.Drawing.Font]::new("Microsoft Sans Serif", 12, [System.Drawing.FontStyle]::Bold)

$objForm.Controls.Add($Label0)
$objForm.Controls.Add($Label1)
$objForm.Controls.Add($Label2)
$objForm.Controls.Add($Label3)
$objForm.controls.add($pictureBox)
$objForm.controls.add($pictureBoxResult)
$objForm.controls.add($TimeDropdown)


######## Hide PowerShell Console#######
Add-Type -Name Window -Namespace Console -MemberDefinition '
[DllImport("Kernel32.dll")]
public static extern IntPtr GetConsoleWindow();
[DllImport("user32.dll")]
public static extern bool ShowWindow(IntPtr hWnd, Int32 nCmdShow);
'
$consolePtr = [Console.Window]::GetConsoleWindow()
[Console.Window]::ShowWindow($consolePtr, 0)

[void] $objForm.ShowDialog()
