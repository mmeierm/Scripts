...
foreach ($Autopilot in $AutopilotData)
{
$Autopilot = $Autopilot | ConvertTo-Json
$DeviceResult=SendIntuneData-CosmosDb -EndPoint $CosmosDBEndPoint -DataBaseId $DataBaseId -CollectionId "IntuneAutopilotContainer" -MasterKey $MasterKey -JSON $Autopilot
} 
